...
SELECT COUNT( name ) AS count_free_facility
FROM Facilities
WHERE membercost =0
...
SELECT CONCAT_WS( ' ', firstname, surname ) AS member_name, f.name AS tennis_facility
FROM Bookings AS b
INNER JOIN Members AS m ON m.memid = b.memid
INNER JOIN Facilities AS f ON b.facid = f.facid
WHERE f.name LIKE '%Tennis Court%'
GROUP BY member_name, tennis_facility
ORDER BY member_name, tennis_facility
LIMIT 0 , 1000;
...
SELECT f.name AS facility, CONCAT_WS(' ', firstname, surname) AS name, b.slots *
...
SELECT CONCAT_WS(', ', m1.surname, m1.firstname) AS member, CONCAT_WS(', ', m2.surname, m2.firstname) AS recommended_by
FROM `Members` AS m1
...
SELECT name, COUNT(memid) AS member_usage
FROM Bookings
INNER JOIN Facilities
USING(facid)
WHERE memid != 0
GROUP BY name
ORDER BY member_usage;

/* Q13: Find the facilities usage by month, but not guests */

SELECT name, MONTH(starttime) AS month, ROUND((SUM(slots*30))/60, 1) AS hours
